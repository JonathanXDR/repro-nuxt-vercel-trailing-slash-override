...
> Uses the Nitro 🐞 Bug report template. Searched existing issues — closest is open #4242 (root `/` variant); this trailing-slash sub-route case is undocumented.
...
> StackBlitz/WebContainers can't run the Vercel build/runtime — use a real `vercel build` + deploy to observe it.
